...
class VentoDriver extends Driver {
  ...
  async onInit() {
    ...
    // Extend library's default parameter sizes with extra params not in the enum
    const EXTRA_PARAMS = { 11: 3 }; // Active timer countdown: 3 bytes (sec, min, hour)
    this.modbusClient.parameterSizeResolver = (param) => {
      if (EXTRA_PARAMS[param] !== undefined) return EXTRA_PARAMS[param];
      return Parameter.getSize(param);
    };
      ...
      if (result != null) {
        ...
        return {
          ...
          boost: {
            ...
            deactivationtimer: entries[4].value['0'],
          },
          operationmode: entries[5].value['0'],
          filter: {
            alarm: entries[6].value['0'],
            timer: {
              min: entries[7].value['0'],
              hour: entries[7].value['1'],
              days: entries[7].value['2'],
            },
          },
          humidity: {
            current: entries[8].value['0'],
            sensoractivation: entries[9].value['0'],
            threshold: entries[10].value['0'],
            activated: 0,
          },
          unittype: unittypelabel,
          fan: {
            rpm: entries[12].value['0'],
          },
          timers: {
            mode: entries[13].value['0'],
            countdown: {
              sec: entries[15].value['0'],
              min: entries[15].value['1'],
              hour: entries[15].value['2'],
            },
          },
          alarm: entries[14].value['0'],
        };
      }
